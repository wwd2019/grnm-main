...
After preprocessing and collecting the required files, the gene score data generated by ArchR and Pando are integrated with the scRNA data (R code). The resulting data can be downloaded from  https://pan.baidu.com/s/1JG_h4zM7jZqbdnj6jPScQA?pwd=b9cm. Subsequently, Principal Component Analysis (PCA) is performed on the integrated data for dimensionality reduction. The number of genes is denoted as M, and the number of PCA dimensions is denoted as d. This yields an M × d matrix.
